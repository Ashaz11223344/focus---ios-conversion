...
class DndStartReceiver : BroadcastReceiver() {
                ...
                if (schedule != null && schedule.isEnabled) {
                    ...
                    if (nm.isNotificationPolicyAccessGranted) {
                        ...
                        kotlinx.coroutines.runBlocking {
                            com.example.motivation.data.SettingsDataStore(context).setDndActive(true)
                        }
                    }
                    // Reschedule next occurrence
                    val manager = DndScheduleManager(context)
                    manager.scheduleDndStart(schedule)
                }
            } catch (e: Exception) {
                android.util.Log.e("DndStartReceiver", "Error handling DND start alarm", e)
            } finally {
